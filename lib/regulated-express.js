...
/**
 * express の all メソッドをフック
 */
RegulatedExpress.prototype.all = function() {
    var args = Array.prototype.slice.call(arguments);
    args.unshift('all');
    return this.wrappedMethod.apply(this, args);
}

/**
 * express の get メソッドをフック
 */
...
/**
 * express の param メソッドをラップ
 */
RegulatedExpress.prototype.param = function() {
    return this.wrappedParam.apply(this, arguments);
}

/**
 * express の http メソッドをラップ
 * リクエストの共通処理を実行するとともに
 * 拡張処理を実行
 */
RegulatedExpress.prototype.wrappedMethod = function(method, path, callback) {
    var thisApp                   = this;
    ...
    // メソッド処理のラッパーを作成
    function methodWrapper(req, res, next) {
        var methodNext      = next;
        ...
        // 状態要求のチェック開始
        validateRequire(staticRequire, require, function(err) {
            // 状態要求にエラーがある場合は即座に終了
            if (err) {
                methodNext(new exceptions.RegulatedExpressConditionRequireException(err));
                return;
            }
            // プロファイルのチェック開始
            validateProfile(staticProfile, profile, function(err) {
                // ■ 注意
                // err は validateProfile のコールバック関数の
                // ものを使用していることに注意
                function next() {
                    if (err) {
                        methodNext(new exceptions.RegulatedExpressValidateParamException(err));
                        return;
                    }
                    callback.apply(thisApp, methodArguments);
                }
        ...
        // 状態要求チェック
        function validateRequire(staticRequire, require, callback) {
            // 状態要求がないならチェックしない
            ...
            // 状態要求を作成
            var requireForCheck = {};
            utils.merge(requireForCheck, staticRequire);
            utils.merge(requireForCheck, require);
            ...
            function next(err) {
                ...
                // ■ 注意
                // チェックすべき require のキーに対して staticRequire が
                // 存在しない場合はログを残して次のチェックへ移る.
                // config でコントローラを外したときなどに、
                // そこに書かれた staticRequire が読み込まれなかった場合に
                // 例外とならないようにする処置.
                // 安全側に倒すならキーが無かった次点で例外とすべきだが
                // コントローラを外すと他の経路が影響を受けるという動作も
                // 問題なので, 適切な警告方法を実装するべき.
        ...
        // プロファイルのチェック
        function validateProfile(staticProfile, profile, callback) {
            // プロファイルがないならチェックしない
            ...
            // プロファイルを作成
            var profileForCheck = {};
            utils.merge(profileForCheck, staticProfile);
            utils.merge(profileForCheck, profile);
    ...
    return this.__super__[method](path, methodWrapper);
}
 ...
 * 拡張処理を実行
 */
RegulatedExpress.prototype.wrappedParam = function() {
