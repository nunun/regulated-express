...
RegulatedExpress.prototype.hookMethod = function(method, path, callback) {
    ...
    function callbackWrapper(req, res) {
        ...
        // 状態要求チェックルーチン
        function validateRequire(req, res, requireForCheck, callback) {
            for (var key in requireForCheck) {
                var value = requireForCheck[key];
                if (!value) {
                    continue;
                }
                var err = requireDefine[key](req, res, value);
                if (err) {
                    callback(err);
                    return;
                }
        ...
        validateRequire(req, res, r, function(err) {
            if (err) {
            ...
            req.validateParams(res, p, function(err) {
                function next(err) {
                    if (err) {
                        throw new exceptions.RegulatedExpressValidateParamException(err);
                    }
                    callback.apply(null, wrapperArguments);
                }
